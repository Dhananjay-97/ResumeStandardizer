...
#     result = subprocess.run(["playwright", "install"], shell=True, check=True)

#     # Check if the command completed successfully
#     if result.returncode == 0:
#         print("Playwright installation completed successfully!")
#     else:
#         print(f"Playwright installation failed with return code {result.returncode}")

# except subprocess.CalledProcessError as e:
#     print(f"Error: Playwright installation failed with error code {e.returncode}")
            ...
            if get_resume_button:
                with st.status("Building resume..."):
                    ...
                    if not os.path.exists(resume_path):
                        st.error(f"PDF not found at {resume_path}")
                    else:
                ...
                with resume_col_2:
                    try:
                        ...
                        st.error(f"File not found at {resume_path}. Please ensure the resume was generated correctly.")
                        raise
